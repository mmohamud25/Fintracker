...
function Modal({title,children,onClose,wide=false}){const G=useG();return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.45)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:20}} onClick={e=>e.target===e.currentTarget&&onClose()}><div style={{background:G.card,border:`1px solid ${G.border}`,borderRadius:20,padding:24,width:"100%",maxWidth:wide?700:430,maxHeight:"90vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,.2)"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><span style={{fontWeight:700,fontSize:16,color:G.text}}>{title}</span><button onClick={onClose} style={{background:"none",border:"none",color:G.muted,fontSize:22,cursor:"pointer",lineHeight:1}}>×</button></div>{children}</div></div>;}
...
function Dashboard({transactions,subscriptions,goals,netWorthHistory}){
  const G=useG();const now=new Date();const tm=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,"0")}`;const pm=`${now.getFullYear()}-${String(now.getMonth()).padStart(2,"0")}`;
...
function Transactions({transactions,setTransactions}){
  const G=useG();const [modal,setModal]=useState(false);const [csvModal,setCsvModal]=useState(false);const [filter,setFilter]=useState("all");const [search,setSearch]=useState("");
  const [form,setForm]=useState({date:"",desc:"",amount:"",type:"expense",category:"Food",note:""});const [csvData,setCsvData]=useState(null);const [mapping,setMapping]=useState({date:"",desc:"",amount:"",type:"",category:""});const [csvMsg,setCsvMsg]=useState("");const fileRef=useRef();
  ...
  const add=async()=>{if(!form.date||!form.desc||!form.amount)return;const u=[{id:uid(),...form,amount:parseFloat(form.amount)},...transactions];setTransactions(u);await store.set("transactions",u);setModal(false);setForm({date:"",desc:"",amount:"",type:"expense",category:"Food",note:""});};
  const del=async id=>{const u=transactions.filter(t=>t.id!==id);setTransactions(u);await store.set("transactions",u);};
...
function Budget({transactions,budgets,setBudgets}){
  const G=useG();const [editing,setEditing]=useState(null);const [newLimit,setNewLimit]=useState("");
  const now=new Date();const tm=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,"0")}`;
  ...
  const save=async cat=>{const v=parseFloat(newLimit);if(isNaN(v)||v<=0)return;const u=budgets.map(b=>b.category===cat?{...b,limit:v}:b);setBudgets(u);await store.set("budgets",u);setEditing(null);setNewLimit("");};
  const total=budgets.reduce((a,b)=>a+b.limit,0);const spent=budgets.reduce((a,b)=>a+getSpent(b.category),0);
  return(
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><h2 style={{fontSize:22,fontWeight:700,color:G.text,marginBottom:3}}>Monthly Budget</h2><p style={{color:G.muted,fontSize:13}}>{now.toLocaleString("default",{month:"long",year:"numeric"})}</p></div><div style={{textAlign:"right"}}><div style={{fontFamily:"monospace",fontSize:18,fontWeight:700,color:spent>total?G.red:G.teal}}>{fmt(spent)} <span style={{color:G.muted,fontSize:13}}>/ {fmt(total)}</span></div></div></div>
...
function Subscriptions({subscriptions,setSubscriptions}){
  const G=useG();const [modal,setModal]=useState(false);const [form,setForm]=useState({name:"",amount:"",due:"1",category:"Entertainment"});const [alertMsg,setAlertMsg]=useState("");
  const total=subscriptions.reduce((a,b)=>a+b.amount,0);const today=new Date().getDate();
  const add=async()=>{if(!form.name||!form.amount)return;const u=[...subscriptions,{id:uid(),...form,amount:parseFloat(form.amount),due:parseInt(form.due)}];setSubscriptions(u);await store.set("subscriptions",u);setModal(false);setForm({name:"",amount:"",due:"1",category:"Entertainment"});};
  const del=async id=>{const u=subscriptions.filter(s=>s.id!==id);setSubscriptions(u);await store.set("subscriptions",u);};
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}><StatCard label="Monthly" value={fmt(total)} color={G.red} icon="🔄"/><StatCard label="Annual" value={fmt(total*12)} color={G.gold} icon="📅"/><StatCard label="Daily" value={fmt(total/30)} color={G.purple} icon="📊"/></div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:12}}>{subscriptions.map(s=>{const b=badge(s.due);return <Card key={s.id}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}><div><div style={{fontWeight:700,fontSize:16,marginBottom:6,color:G.text}}>{s.name}</div><Pill label={s.category} color={CAT_COLOR[s.category]||G.muted}/>{b&&<div style={{marginTop:6}}><Pill label={b.label} color={b.color}/></div>}<div style={{marginTop:10,color:G.muted,fontSize:11}}>Day {s.due} monthly</div></div><div style={{textAlign:"right"}}><div style={{fontFamily:"monospace",fontSize:20,fontWeight:700,color:G.red}}>{fmt(s.amount)}</div><div style={{fontSize:11,color:G.muted,marginBottom:10}}>/month</div><button onClick={()=>del(s.id)} style={{background:"none",border:`1px solid ${G.border}`,color:G.muted,borderRadius:6,padding:"4px 10px",cursor:"pointer",fontSize:11}}>Remove</button></div></div></Card>;})}
      </div>
...
function Goals({goals,setGoals}){
  const G=useG();const [modal,setModal]=useState(false);const [dep,setDep]=useState({id:null,amount:""});const [form,setForm]=useState({name:"",target:"",saved:"0",icon:"🎯"});
  const ICONS=["🎯","✈️","🏠","💻","🚗","💍","🎓","🛡️","📱","🏋️","🌴","💰"];const COLS=[G.teal,G.gold,G.purple,G.red,"#EA580C","#0284C7","#16A34A"];
  const add=async()=>{if(!form.name||!form.target)return;const u=[...goals,{id:uid(),...form,target:parseFloat(form.target),saved:parseFloat(form.saved||0)}];setGoals(u);await store.set("goals",u);setModal(false);setForm({name:"",target:"",saved:"0",icon:"🎯"});};
  const addDep=async()=>{const v=parseFloat(dep.amount);if(isNaN(v)||v<=0)return;const u=goals.map(g=>g.id===dep.id?{...g,saved:Math.min(g.target,g.saved+v)}:g);setGoals(u);await store.set("goals",u);setDep({id:null,amount:""});};
  const del=async id=>{const u=goals.filter(g=>g.id!==id);setGoals(u);await store.set("goals",u);};
  return(
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
...
/* ── NET WORTH ── */
function NetWorth({assets,setAssets,liabilities,setLiabilities,netWorthHistory,setNetWorthHistory}){
  const G=useG();const [aModal,setAModal]=useState(false);const [lModal,setLModal]=useState(false);const [aForm,setAForm]=useState({name:"",value:"",type:"cash"});const [lForm,setLForm]=useState({name:"",value:"",type:"credit"});
  ...
  const addA=async()=>{if(!aForm.name||!aForm.value)return;const u=[...assets,{id:uid(),...aForm,value:parseFloat(aForm.value)}];setAssets(u);await store.set("assets",u);await snap(u,liabilities);setAModal(false);setAForm({name:"",value:"",type:"cash"});};
  const addL=async()=>{if(!lForm.name||!lForm.value)return;const u=[...liabilities,{id:uid(),...lForm,value:parseFloat(lForm.value)}];setLiabilities(u);await store.set("liabilities",u);await snap(assets,u);setLModal(false);setLForm({name:"",value:"",type:"credit"});};
  const delA=async id=>{const u=assets.filter(a=>a.id!==id);setAssets(u);await store.set("assets",u);await snap(u,liabilities);};
  const delL=async id=>{const u=liabilities.filter(l=>l.id!==id);setLiabilities(u);await store.set("liabilities",u);await snap(assets,u);};
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}><StatCard label="Total Assets" value={fmt(tA)} color={G.teal} icon="📈"/><StatCard label="Total Liabilities" value={fmt(tL)} color={G.red} icon="📉"/><StatCard label="Net Worth" value={fmt(nw)} color={nw>=0?G.teal:G.red} icon="💎"/></div>
...
function CreditCards({cards,setCards}){
  const G=useG();const [modal,setModal]=useState(false);const [payModal,setPayModal]=useState(null);const [calcModal,setCalcModal]=useState(false);
  ...
  const addCard=async()=>{if(!form.name||!form.balance||!form.limit||!form.apr)return;const u=[...cards,{id:uid(),...form,limit:parseFloat(form.limit),balance:parseFloat(form.balance),apr:parseFloat(form.apr),minPayment:parseFloat(form.minPayment||25),dueDay:parseInt(form.dueDay),payments:[]}];setCards(u);await store.set("cards",u);setModal(false);setForm({name:"",last4:"",limit:"",balance:"",apr:"",minPayment:"",dueDay:"15",ca:"#1a1a2e",cb:"#16213e"});};
  const delCard=async id=>{const u=cards.filter(c=>c.id!==id);setCards(u);await store.set("cards",u);};
  const makePay=async()=>{const amt=parseFloat(payAmt);if(isNaN(amt)||amt<=0)return;const u=cards.map(c=>c.id===payModal.id?{...c,balance:Math.max(0,parseFloat((c.balance-amt).toFixed(2))),payments:[{id:uid(),amount:amt,date:new Date().toISOString().slice(0,10)},...(c.payments||[])]}:c);setCards(u);await store.set("cards",u);setPayModal(null);setPayAmt("");};
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:16}}>
        {cards.map(card=>{const avail=card.limit-card.balance;const p=Math.min(100,(card.balance/card.limit)*100);const uc2=p>50?G.red:p>30?G.gold:G.green;const b=dueBadge(card.dueDay);return <div key={card.id} style={{display:"flex",flexDirection:"column",gap:10}}>
          <div style={{background:`linear-gradient(135deg,${card.ca},${card.cb})`,borderRadius:16,padding:"20px 22px",color:"#fff",position:"relative",overflow:"hidden",minHeight:140}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:24}}><div style={{fontWeight:700,fontSize:15}}>{card.name}</div><button onClick={()=>delCard(card.id)} style={{background:"rgba(255,255,255,.15)",border:"none",color:"#fff",borderRadius:6,padding:"3px 8px",cursor:"pointer",fontSize:11}}>✕</button></div>
      ...
      {calcModal&&<Modal title="Payoff Calculator" wide onClose={()=>setCalcModal(false)}><div style={{display:"flex",flexDirection:"column",gap:16}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><Field label="Extra Monthly Payment ($)"><Inp type="number" value={extra} onChange={e=>setExtra(e.target.value)} placeholder="100"/></Field><Field label="Strategy"><Sel value={method} onChange={e=>setMethod(e.target.value)}><option value="avalanche">Avalanche (Highest APR First)</option><option value="snowball">Snowball (Lowest Balance First)</option></Sel></Field></div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>{[{label:"Min Payments Only",r:mn,color:G.red},{label:"Avalanche Method",r:av,color:G.teal},{label:"Snowball Method",r:sn,color:G.purple}].map(({label,r,color})=><div key={label} style={{background:G.card2,border:`1px solid ${G.border}`,borderRadius:12,padding:14}}><div style={{fontSize:10,color:G.muted,fontWeight:700,letterSpacing:.5,marginBottom:8}}>{label.toUpperCase()}</div><div style={{fontFamily:"monospace",fontSize:18,fontWeight:700,color,marginBottom:4}}>{r.months?fmtM(r.months):"Paid off!"}</div><div style={{fontSize:11,color:G.muted}}>Interest: {fmt(r.interest)}</div></div>)}</div>
...
function Settings({onClose,isDark,setIsDark,allData,onLock,onClearData}){
  const G=useG();const [msg,setMsg]=useState("");const fileRef=useRef();
  const say=(m,t=3500)=>{setMsg(m);setTimeout(()=>setMsg(""),t);};
  const handleImport=e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=async ev=>{try{const d=JSON.parse(ev.target.result);for(const k of["transactions","budgets","subscriptions","goals","assets","liabilities","cards"])if(d[k])await store.set(k,d[k]);say("✅ Imported! Refresh the page to load the new data.");}catch{say("❌ Invalid backup file.");}};r.readAsText(f);};
  return(
    <Modal title="⚙️ Settings" onClose={onClose}><div style={{display:"flex",flexDirection:"column",gap:20}}>
      {msg&&<div style={{background:`${G.teal}12`,border:`1px solid ${G.teal}30`,borderRadius:10,padding:"10px 14px",fontSize:12,color:G.teal}}>{msg}</div>}
      ...
      <div><div style={{fontWeight:600,fontSize:13,color:G.text,marginBottom:12}}>Security</div><Btn outline onClick={onLock} style={{width:"100%"}}>🔒 Lock App Now</Btn></div>
      <div><div style={{fontWeight:600,fontSize:13,color:G.text,marginBottom:4}}>Cloud Sync</div><div style={{background:`${G.gold}10`,border:`1px solid ${G.gold}30`,borderRadius:10,padding:"12px 14px",fontSize:12,color:G.muted}}>☁️ <strong style={{color:G.text}}>Coming soon:</strong> Supabase cloud sync will let you access your data from any device. Connect your Supabase project in a future update.</div></div>
...
export default function App(){
  ...
  const clearAll=async()=>{if(!confirm("Delete ALL data? This cannot be undone."))return;for(const k of["transactions","budgets","subscriptions","goals","assets","liabilities","cards","netWorthHistory","pin"])await store.set(k,null);setTransactions(SEED_TXN);setBudgets(SEED_BUDGETS);setSubscriptions(SEED_SUBS);setGoals(SEED_GOALS);setAssets(SEED_ASSETS);setLiabilities(SEED_LIAB);setCards(SEED_CARDS);setNetWorthHistory([]);setPinSet(false);setLocked(true);setSettingsOpen(false);};
  ...
  return(
    <ThemeCtx.Provider value={G}>
      ...
      <div style={{display:"flex",minHeight:"100vh",background:G.bg}}>
        <div style={{width:215,flexShrink:0,background:G.sidebar,borderRight:`1px solid ${G.border}`,display:"flex",flexDirection:"column",position:"fixed",top:0,bottom:0,left:0,boxShadow:"2px 0 8px rgba(0,0,0,.05)"}}>
        ...
        <div style={{flex:1,marginLeft:215,padding:"28px 32px",maxWidth:"calc(100vw - 215px)"}}>
          <div className="fade" key={tab}>
            ...
            {tab==="subscriptions"&&<Subscriptions subscriptions={subscriptions} setSubscriptions={setSubscriptions}/>}
            {tab==="goals"        &&<Goals goals={goals} setGoals={setGoals}/>}
            {tab==="networth"     &&<NetWorth assets={assets} setAssets={setAssets} liabilities={liabilities} setLiabilities={setLiabilities} netWorthHistory={netWorthHistory} setNetWorthHistory={setNetWorthHistory}/>}
            {tab==="cards"        &&<CreditCards cards={cards} setCards={setCards}/>}
          </div>
        </div>
      </div>
