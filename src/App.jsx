...
function Transactions({transactions,setTransactions}){
  const G=useG();const [modal,setModal]=useState(false);const [editTxn,setEditTxn]=useState(null);const [csvModal,setCsvModal]=useState(false);const [filter,setFilter]=useState("all");const [search,setSearch]=useState("");
  ...
  const del=async id=>{if(!confirm("Delete this transaction?"))return;const u=transactions.filter(t=>t.id!==id);setTransactions(u);await store.set("transactions",u);};
  const openEdit=t=>{setEditTxn({...t,amount:String(t.amount)});};
  const saveEdit=async()=>{if(!editTxn.date||!editTxn.desc||!editTxn.amount)return;const u=transactions.map(t=>t.id===editTxn.id?{...editTxn,amount:parseFloat(editTxn.amount)}:t);setTransactions(u);await store.set("transactions",u);setEditTxn(null);};
...
function Subscriptions({subscriptions,setSubscriptions}){
  const G=useG();const [modal,setModal]=useState(false);const [editSub,setEditSub]=useState(null);const [form,setForm]=useState({name:"",amount:"",due:"1",category:"Entertainment"});const [alertMsg,setAlertMsg]=useState("");
  ...
  const del=async id=>{if(!confirm("Remove this subscription?"))return;const u=subscriptions.filter(s=>s.id!==id);setSubscriptions(u);await store.set("subscriptions",u);};
  const saveEdit=async()=>{if(!editSub.name||!editSub.amount)return;const u=subscriptions.map(s=>s.id===editSub.id?{...editSub,amount:parseFloat(editSub.amount),due:parseInt(editSub.due)}:s);setSubscriptions(u);await store.set("subscriptions",u);setEditSub(null);};
...
function Goals({goals,setGoals}){
  ...
  const del=async id=>{if(!confirm("Delete this goal?"))return;const u=goals.filter(g=>g.id!==id);setGoals(u);await store.set("goals",u);};
  const saveEdit=async()=>{if(!editGoal.name||!editGoal.target)return;const u=goals.map(g=>g.id===editGoal.id?{...editGoal,target:parseFloat(editGoal.target),saved:parseFloat(editGoal.saved||0)}:g);setGoals(u);await store.set("goals",u);setEditGoal(null);};
  return(
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
...
function NetWorth({assets,setAssets,liabilities,setLiabilities,netWorthHistory,setNetWorthHistory}){
  ...
  const [editItem,setEditItem]=useState(null);// {item, kind:'asset'|'liability'}
  ...
  const delA=async id=>{if(!confirm("Delete this asset?"))return;const u=assets.filter(a=>a.id!==id);setAssets(u);await store.set("assets",u);await snap(u,liabilities);};
  const delL=async id=>{if(!confirm("Delete this liability?"))return;const u=liabilities.filter(l=>l.id!==id);setLiabilities(u);await store.set("liabilities",u);await snap(assets,u);};
  const saveEditItem=async()=>{const v=parseFloat(editItem.item.value);if(!editItem.item.name||isNaN(v))return;if(editItem.kind==="asset"){const u=assets.map(a=>a.id===editItem.item.id?{...editItem.item,value:v}:a);setAssets(u);await store.set("assets",u);await snap(u,liabilities);}else{const u=liabilities.map(l=>l.id===editItem.item.id?{...editItem.item,value:v}:l);setLiabilities(u);await store.set("liabilities",u);await snap(assets,u);}setEditItem(null);};
...
function CreditCards({cards,setCards}){
  ...
  const delCard=async id=>{if(!confirm("Delete this card?"))return;const u=cards.filter(c=>c.id!==id);setCards(u);await store.set("cards",u);};
  const saveEditCard=async()=>{if(!editCard.name||!editCard.limit||!editCard.apr)return;const u=cards.map(c=>c.id===editCard.id?{...editCard,limit:parseFloat(editCard.limit),balance:parseFloat(editCard.balance),apr:parseFloat(editCard.apr),minPayment:parseFloat(editCard.minPayment||25),dueDay:parseInt(editCard.dueDay)}:c);setCards(u);await store.set("cards",u);setEditCard(null);};
